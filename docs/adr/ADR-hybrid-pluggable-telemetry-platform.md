...
| Traccar | production HTTPS polling | environment-backed opaque bearer token | implemented / real-provider acceptance pending |
...
| `US-55-TRACCAR-ADAPTER` production polling | COMPLETE; qualified Traccar 6.15.3 provider SPI, secret resolver and canonical Kafka boundary | No migration, new permission or public API; deployment-only private endpoint allowlist | Bearer auth, bounded history, overflow, DNS/SSRF, watermark, normalization, architecture and full regression gates pass; real-provider evidence remains pending |
