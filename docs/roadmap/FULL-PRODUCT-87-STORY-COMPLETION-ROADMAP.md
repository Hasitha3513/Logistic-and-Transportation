...
**Current state:** 65 / 87 accepted; 22 / 87 remaining  
...
`US-35, US-37, US-38, US-46, US-47, US-48, US-49, US-50, US-51, US-52, US-53, US-54, US-55, US-72, US-73, US-76, US-78, US-82, US-84, US-85, US-86, US-87`.

This is 22 stories, so the invariant remains `65 + 22 = 87`. `US-88`, `US-89`, and `US-90` are undefined and must not be created. The word *deferred* now means scheduled into a governed future wave, not permanently abandoned.
...
The highest-leverage current story is **US-73 — Manage External Integrations**. Its frozen boundary is implemented and independently technically closed with one governed outbound JSON-file adapter, controlled-sandbox evidence, and provider-neutral reliability without duplicating the P1-01 outbox. It remains acceptance-pending until independent final acceptance; later stories may separately approve payroll exchange, billing/accounting handoff, fuel-card imports, telematics, DMS/OCR, push/webhooks, and third-party failure handling.
...
## 4. Authoritative 22-story register
...
| US-73 | Manage External Integrations | justified `integration` bounded context | IMPLEMENTATION_COMPLETE / ACCEPTANCE_PENDING | P0 | A | H/H/H/H/H | Identity, Tenancy, Audit, US-75, P1-01; domain-owned mappings | Controlled filesystem sandbox; no vendor | IMPLEMENTED: governed outbound JSON file; environment-backed credential-reference port; versioned declarative mapping; five-attempt retry; read-only reconciliation | YES | new admin API; internal durable fact contract; no public inbound API | OPERATOR UI | RBAC/SoD; Tenant; secret/path/payload redaction; future sensitive types rejected | Connectivity/reliable exchange only; no business meaning; reuses P1-01, no cloned outbox | US-78 decisions | Technical closure PASS: V61, focused 24/24, regression 40/40, Maven 1,276/0/0/15, architecture 44/44, Chromium 6/6 | COMPLETE — decision, implementation, and technical evidence cover owner, adapter, security, persistence, API/UI and controlled sandbox | Configure/test/enable/exchange/status/retry pass; failures never appear synced; DoD-C pending independent final acceptance |
...
| US-78 | Manage Operational Exceptions | justified `operations` exception-lifecycle context | READY_FOR_PRODUCT_DECISIONS | P0 | A | H/H/H/M/H | US-15/23/30/38/45/55/62/68 detectors, US-75 Audit, US-77 Notification, US-80 Workflow, US-81 Scheduling, P1-01 where durable | NONE | incident versus projection; intake contract; taxonomy; assignment; SLA; escalation; RCA/closure; retention | YES | central lifecycle APIs + typed intake events/ports | OPERATOR UI | RBAC/ABAC/SoD; Tenant; potentially sensitive evidence | Central aggregate owns lifecycle only; must not duplicate domain exception data or mutate source aggregates | US-73 decisions | product/ownership decision; detectors integrate incrementally | Aggregate/projection choice, typed intake, owner references, SLA/closure and permission contracts frozen | Classification/severity/assignment/SLA/escalation/corrective action/RCA/closure pass without detector duplication; DoD-C |
...
1. `US-73-EXTERNAL-INTEGRATIONS-FINAL-ACCEPTANCE-001` (implementation and technical closure complete; count unchanged until acceptance).
2. `US-78-OPERATIONAL-EXCEPTIONS-PRODUCT-DECISIONS-001` → US-78 implementation → closure → acceptance (decision work may overlap item 1).
...
`US-73-EXTERNAL-INTEGRATIONS-FINAL-ACCEPTANCE-001`

It must independently assess only the frozen governed outbound JSON-file capability; it must not promise every ecosystem named in the mind map.
