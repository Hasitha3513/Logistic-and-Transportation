...
The highest-leverage next story is **US-73 — Manage External Integrations**. Its product decisions must freeze a deliberately small acceptance boundary and provider-neutral reliability model. It can then support payroll exchange, billing/accounting handoff, fuel-card imports, telematics, DMS/OCR, push/webhooks, and third-party failure handling without duplicating the P1-01 outbox.
...
| US-73 | Manage External Integrations | justified `integration` bounded context | READY_FOR_PRODUCT_DECISIONS | P0 | A | H/H/H/H/H | Identity, Tenancy, Audit, US-75, P1-01; domain-owned mappings | At least one real sandbox or governed file endpoint | minimum accepted integration types; credentials; mappings; sync direction; retry/idempotency; operator actions; SLA; webhook/file security | YES | new admin API + webhook/file/external adapters | OPERATOR UI | RBAC/SoD; Tenant; secrets; PII/financial/location payloads; SSRF/webhook threats | Connectivity/reliable exchange only; no business meaning; reuse P1-01, never clone outbox | US-78 decisions | product decisions; sandbox for real-integration claim | One minimum end-to-end integration boundary, security, mapping, durability, failure and sandbox evidence frozen | Configure/test/enable/exchange/status/retry pass; failures never appear synced; real sandbox/file evidence; DoD-C |
...
1. `US-73-EXTERNAL-INTEGRATIONS-PRODUCT-DECISIONS-001` → US-73 implementation → closure → acceptance.
...
`US-73-EXTERNAL-INTEGRATIONS-PRODUCT-DECISIONS-001`

It must freeze a minimum authoritative acceptance boundary; it must not promise every ecosystem named in the mind map.
