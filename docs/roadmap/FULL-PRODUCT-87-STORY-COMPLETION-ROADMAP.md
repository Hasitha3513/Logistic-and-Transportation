...
**Wave A is 2 / 2 COMPLETE / CLOSED and Wave B is OPEN.** US-73 provides one accepted governed outbound JSON-file adapter with controlled-sandbox evidence. US-78 is independently accepted with Routing and Delivery flowing through one Operations lifecycle while preserving source ownership and P1-01 durability. US-37 final acceptance passes and US-35 product decisions are frozen; the next task is `US-35-FUEL-CARDS-IMPLEMENTATION-001`.
...
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| US-35 | Manage Fuel Cards | `fuel` | PRODUCT_DECISIONS_FROZEN / IMPLEMENTATION_NOT_STARTED | P1 | B | H/H/H/H/H | US-31/32/34/36, Fleet, Driver, Organization, Identity, Tenancy, Audit; US-73 outbound capability inspected but not reused | NONE for controlled fixture; named provider remains future | Frozen: limited card reference; local lifecycle; one Vehicle/Driver binding; limits; canonical JSON import; immutable transactions; deterministic reconciliation/review; no payment/fraud engine | LIKELY; next free version after V63 | frozen operator card/import/reconciliation API | OPERATOR UI | five permissions; Tenant; masked data; importer/reconciler SoD | Fuel owns local control/evidence only; provider owns financial authority; no foreign persistence | US-46 decisions | NONE | COMPLETE — `US-35-FUEL-CARDS-PRODUCT-DECISIONS-001.md` | Issue/restrict/import/reconcile/review AC pass; real controlled JSON parsing/persistence/dedupe; source immutability; DoD-C |
...
`US-35-FUEL-CARDS-IMPLEMENTATION-001`
