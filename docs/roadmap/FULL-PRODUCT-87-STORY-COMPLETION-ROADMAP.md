...
**Current state:** 66 / 87 accepted; 21 / 87 remaining
...
**US-73 — Manage External Integrations** is independently accepted and COMPLETE. Its frozen boundary provides one governed outbound JSON-file adapter with controlled-sandbox evidence and provider-neutral reliability without duplicating the P1-01 outbox. Acceptance does not claim that payroll, billing/accounting, fuel-card, telematics, DMS/OCR, push/webhook, or other named ecosystems are connected. The current Wave A focus is US-78 product decisions.
...
2. `US-78-OPERATIONAL-EXCEPTIONS-PRODUCT-DECISIONS-001` → US-78 implementation → closure → acceptance.
...
`US-78-OPERATIONAL-EXCEPTIONS-PRODUCT-DECISIONS-001`

It must freeze the central operational-exception lifecycle without taking detector ownership or inventing cross-module repository access.
