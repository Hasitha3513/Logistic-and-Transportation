...
| US-53 | Replay Journeys | `tracking` | PRODUCT_DECISIONS_COMPLETE / READY_FOR_IMPLEMENTATION | P2 | C | M/M/H/M/H | US-48 retained history; US-49/50/52 labelled overlays; Trip; Routing; Audit | Physical/provider evidence only for final acceptance | 7-day bounded chronological replay; deterministic 3 km/h, 5-minute, 50 m stop rule; explicit gaps/quality; no export or engine inference | V93 expected only for two permissions; no replay projection | three read-only body-query endpoints under `/api/v1/tracking/journey-replays` | OPERATOR UI with map/timeline/playback/stops | `JOURNEY_REPLAY_VIEW`, `JOURNEY_REPLAY_INCIDENT_VIEW`; Tenant; no-store/location privacy | Timescale history is authoritative; published Trip/Routing only; overlays retain producer status | US-55 after core overlays | stable V86/V87 history; accepted US-49 and technical-only US-50/52 overlays | Decisions frozen; implementation starts CS01 | Core replay may proceed without US-48/50/52 physical acceptance; own final evidence required |
...
`US-53-REPLAY-JOURNEYS-CS01-DOMAIN-QUERY-CONTRACTS-001`

US-48, US-50 and US-52 remain on independent external acceptance holds. US-49 is COMPLETE / ACCEPTED and accounting remains 73 / 87. US-53 product decisions are frozen: immutable Timescale history is authoritative, replay and stop analysis are bounded, optional overlays retain producer status, and export/engine inference are excluded. Implementation starts with `US-53-REPLAY-JOURNEYS-CS01-DOMAIN-QUERY-CONTRACTS-001`; Flyway remains V92 and V93 is reserved only for the later permission seed.
