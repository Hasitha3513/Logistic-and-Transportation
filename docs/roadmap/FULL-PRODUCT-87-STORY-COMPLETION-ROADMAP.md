...
**Architecture enabler:** `P1-01` COMPLETE; current Flyway head `V100`
...
| US-51 | Monitor Idle Time | `tracking` | DECISION_READY_FOR_APPROVAL / IMPLEMENTATION_NOT_AUTHORIZED | P1 | C | M/M/M/M/M | US-48 canonical history/movement, effective device capabilities, Audit; Fuel estimate deferred | Production engine-running activation and final acceptance require verified native device evidence; technical fixtures do not | PROPOSED D1–D11: combustion/hybrid; native engine-running; 3 km/h, adjusted 50 m, five minutes/two samples, two-minute gap; no Phase-1 effects/fuel estimate | Proposed V101–V103, unreserved | four proposed read-only `/api/v1/tracking/idle-monitoring` routes | proposed OPERATOR state/history/detail UI | proposed `IDLE_MONITOR_VIEW`, `IDLE_EVENT_VIEW`; Tenant; no coordinates/PII/provider raw facts | Tracking owns detection/evidence; V1/V2 ignition remains immutable/ineligible; no foreign persistence | US-49, US-50, US-52 | D1–D11 product-owner approval; production activation separately needs verified engine-running mapping | Consolidated package resolves software contract; first proposed task is CS01, with production mappings disabled | Technical fixtures may close software only; physical evidence/operator sign-off required for story acceptance |
...
US-51 prerequisite/product-decision work is now selected under the newly created
`US-51-MONITOR-IDLE-TIME-PREREQUISITE-AND-PRODUCT-DECISIONS-001`. Its consolidated package is
implementation-ready for review but remains proposed: canonical V3, migrations, permissions, APIs
and event changes await explicit approval. Software implementation may proceed after that approval
with test-profile fixtures while every production engine-running mapping remains unavailable until
separately verified. The proposed first change set is
`US-51-MONITOR-IDLE-TIME-CS01-CANONICAL-ENGINE-SEMANTICS-001`.

US-48, US-50, US-52, US-53, US-54 and US-55 remain on independent external acceptance holds. US-49 is COMPLETE / ACCEPTED and accounting remains 73 / 87. US-55 field acceptance remains openly queued at `US-55-HANDLE-GPS-EDGE-CASES-FINAL-ACCEPTANCE-001` with 9 externally blocked requirements. Canonical polling, the bounded Traccar 6.15.3 adapter, guided Flespi/Traccar provider UI, safe provider health recovery, idempotency closure and post-extension technical closure are complete; Generic signed-HMAC continuity is preserved and real-provider evidence remains absent. US-51 implementation awaits D1–D11 approval, not physical hardware; production source activation and final physical acceptance remain separate holds. Flyway is V100.
