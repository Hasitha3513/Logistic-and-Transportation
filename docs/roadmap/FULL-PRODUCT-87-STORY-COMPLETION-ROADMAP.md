...
**Current state:** 73 / 87 accepted; 14 / 87 remaining; Waves A and B are COMPLETE / CLOSED; Wave C is active; US-48, US-50, US-52 and US-53 are implementation complete with independent physical acceptance holds; US-49 is COMPLETE / ACCEPTED
...
| US-54 | View Tracking Dashboard | `tracking` | IMPLEMENTATION_IN_PROGRESS / CS05_COMPLETE | P2 | C | H/L/H/M/M | US-48..53 technical producer contracts, published bulk Trip context, Notification own unread API | Existing map component/provider only; physical telemetry/operator evidence for final acceptance | bounded fleet summary/map/table; observed-speed motion; no idle; 24-hour labelled incidents; 0.01° live/recent density; 15-second visible polling | V94 permission seed; V95 Tenant-leading recent-speed index; no projection | implemented read-only `POST /api/v1/tracking/dashboard/query`; body filters, Tenant-bound cursor, no-store | responsive permission-gated dashboard, table/map fallback, density layer, polling/backoff and producer labels complete | conjunctive dashboard/coordinate/incident/replay permissions; Tenant; no PII/device/provider facts | Tracking aggregate uses Tracking ports and one published bulk Trip query; Notification queried independently | CS01–CS05 complete; PostgreSQL load, Redis degradation/recovery and operational metrics pass | Product decision frozen; no physical acceptance inheritance | Execute `US-54-VIEW-TRACKING-DASHBOARD-TECHNICAL-CLOSURE-001` | Technical/degraded and physical/operator acceptance remain separate |
...
US-48, US-50, US-52 and US-53 remain on independent external acceptance holds. US-49 is COMPLETE / ACCEPTED and accounting remains 73 / 87. The deferred US-53 task is `US-53-REPLAY-JOURNEYS-FINAL-ACCEPTANCE-001`; resume it only when genuine retained provider/device journey evidence and operator sign-off exist. US-54 CS05 is complete: V95 adds the Tenant-leading recent-speed index; PostgreSQL load, Redis degradation/recovery, bounded metrics and the retained 6/6 Chromium suite pass. Technical closure is next.
