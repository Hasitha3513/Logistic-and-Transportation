...
| US-51 | Monitor Idle Time | `tracking` | IMPLEMENTATION_IN_PROGRESS / CS03_COMPLETE | P1 | C | M/M/M/M/M | US-48 canonical history/movement, effective device capabilities, Audit; Fuel estimate deferred | Production engine-running activation and final acceptance require verified native device evidence; technical fixtures do not | APPROVED D1–D11: combustion/hybrid; native engine-running; 3 km/h, adjusted 50 m, five minutes/two samples, two-minute gap; no Phase-1 effects/fuel estimate | V102 verified: immutable V3 history, effective-dated ENGINE_RUNNING vocabulary, idle state/episode/append-only evidence and staged durable dispatch; no production capability seeded | four approved read-only `/api/v1/tracking/idle-monitoring` routes, not yet implemented | approved OPERATOR state/history/detail UI, not yet implemented | approved `IDLE_MONITOR_VIEW`, `IDLE_EVENT_VIEW`; Tenant; no coordinates/PII/provider raw facts | Tracking owns detection/evidence; V1/V2 ignition remains immutable/ineligible; V3 contract complete but production gated | US-49, US-50, US-52 | CS04 evaluator next; production activation separately needs verified engine-running mapping | CS03 migration, concurrency, persistence and staged lease evidence passes; mandatory backend and architecture gates PASS | Technical fixtures may close software only; physical evidence/operator sign-off required for story acceptance |
