...
**Architecture enabler:** `P1-01` COMPLETE; Flyway head `V60`  
...
The highest-leverage current story is **US-73 — Manage External Integrations**. Its product decisions are frozen in `docs/full-product/US-73-EXTERNAL-INTEGRATIONS-PRODUCT-DECISIONS-001.md`: one governed outbound JSON-file adapter, controlled-sandbox evidence, and provider-neutral reliability without duplicating the P1-01 outbox. It can later support payroll exchange, billing/accounting handoff, fuel-card imports, telematics, DMS/OCR, push/webhooks, and third-party failure handling.
...
| US-73 | Manage External Integrations | justified `integration` bounded context | PRODUCT_DECISIONS_FROZEN / IMPLEMENTATION_NOT_STARTED | P0 | A | H/H/H/H/H | Identity, Tenancy, Audit, US-75, P1-01; domain-owned mappings | Controlled filesystem sandbox; no vendor | FROZEN: governed outbound JSON file; environment-backed credential-reference port; versioned declarative mapping; five-attempt retry; read-only reconciliation | YES | new admin API; internal durable fact contract; no public inbound API | OPERATOR UI | RBAC/SoD; Tenant; secret/path/payload redaction; future sensitive types rejected | Connectivity/reliable exchange only; no business meaning; reuse P1-01, never clone outbox | US-78 decisions | implementation not started | COMPLETE — decision document freezes owner, adapter, security, persistence, API/UI and controlled-sandbox acceptance | Configure/test/enable/exchange/status/retry pass; failures never appear synced; real sandbox/file evidence; DoD-C |
...
1. `US-73-EXTERNAL-INTEGRATIONS-IMPLEMENTATION-001` → technical closure → independent final acceptance (product decisions frozen).
...
`US-73-EXTERNAL-INTEGRATIONS-IMPLEMENTATION-001`
