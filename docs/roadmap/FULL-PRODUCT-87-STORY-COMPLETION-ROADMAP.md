...
| US-54 | View Tracking Dashboard | `tracking` | IMPLEMENTATION_IN_PROGRESS / CS03_COMPLETE | P2 | C | H/L/H/M/M | US-48..53 technical producer contracts, published bulk Trip context, Notification own unread API | Existing map component/provider only; physical telemetry/operator evidence for final acceptance | bounded fleet summary/map/table; observed-speed motion; no idle; 24-hour labelled incidents; 0.01° live/recent density; 15-second visible polling | V94 seeds `TRACKING_DASHBOARD_VIEW` only; no projection | implemented read-only `POST /api/v1/tracking/dashboard/query`; body filters, Tenant-bound cursor, no-store | responsive dashboard with map/table fallback and producer labels is next | conjunctive dashboard/coordinate/incident/replay permissions; Tenant; no PII/device/provider facts | Tracking aggregate uses Tracking ports and one published bulk Trip query; Notification queried independently | CS01 contracts, CS02 bounded aggregation and CS03 secured API/RBAC/audit complete | Product decision frozen; no physical acceptance inheritance | Execute `US-54-VIEW-TRACKING-DASHBOARD-CS04-FRONTEND-001` | Technical/degraded and physical/operator acceptance remain separate |
