...
**Current state:** 71 / 87 accepted; 16 / 87 remaining; Wave A is 2 / 2 COMPLETE / CLOSED; Wave B is OPEN
...
`US-47, US-48, US-49, US-50, US-51, US-52, US-53, US-54, US-55, US-72, US-76, US-82, US-84, US-85, US-86, US-87`.

This is 16 stories, so the invariant is `71 + 16 = 87`. `US-88`, `US-89`, and `US-90` are undefined and must not be created. The word *deferred* means scheduled into a governed future wave, not permanently abandoned.
...
**Wave A is 2 / 2 COMPLETE / CLOSED and Wave B is OPEN.** US-73 and US-78 are accepted. US-35, US-37, US-38, and US-46 final acceptance pass. US-47 implementation is complete with acceptance pending and no accounting change. The next task is `US-47-TRANSPORT-BILLING-TECHNICAL-CLOSURE-001`.
...
| US-47 | Manage Transport Billing | dedicated `billing` bounded context; Finance owns tax invoice/ledger/AR/posting/payment | IMPLEMENTATION_COMPLETE / ACCEPTANCE_PENDING | P1 | B | H/H/H/H/H | Closed Trip and explicit Freight completed/closed owner facts; Customer/Organization; Audit; US-72 gate; P1-01; US-73 | Controlled `FILE_JSON_V1`; no live accounting system or acknowledgement | Per-source `TransportBillingRecord`; explicit base/surcharge/penalty/credit lines; supplied tax facts; required cost-centre allocation; approval/finalization/reversal; durable idempotency | V72 | exact `/api/v1/billing/records` command/query family | OPERATOR UI under AppLayout | five permissions; preparer/approver SoD; Tenant; minimized financial/tax data | Finance boundary preserved; no foreign persistence, tax engine, ERP, or Customer duplication | technical closure | NONE | Implementation PASS: deterministic concurrency 9/9, Maven 1,398/0/0/15, architecture 46/46, static/frontend gates, Chromium 7/7, V1→V72 | Technical closure then independent acceptance; accounting unchanged |
...
7. US-47 product/ARB decisions COMPLETE → implementation → closure → acceptance.
...
`US-47-TRANSPORT-BILLING-TECHNICAL-CLOSURE-001`

It must independently verify the implemented US-47 Billing contract and V72 without advancing story accounting.
