...
| US-51 | Monitor Idle Time | `tracking` | IMPLEMENTATION_IN_PROGRESS / CS05_COMPLETE | P1 | C | M/M/M/M/M | US-48 canonical history/movement, effective device capabilities, Audit; Fuel estimate deferred | Production powertrain authority is absent and production lookup returns UNKNOWN; engine-running activation and final acceptance require verified native device evidence | APPROVED D1–D11: combustion/hybrid; native engine-running; 3 km/h, adjusted 50 m, five minutes/two samples, two-minute gap; no Phase-1 effects/fuel estimate | V104 verified: V103 candidate lifecycle retained and exactly two idle read permissions granted only to existing ADMIN/DISPATCHER | four approved read-only `/api/v1/tracking/idle-monitoring` routes implemented with bounded signed cursors, Tenant-safe lookups, no-store and minimized audit | approved OPERATOR state/history/detail UI, not yet implemented | `IDLE_MONITOR_VIEW`, `IDLE_EVENT_VIEW`; independent HTTP/use-case enforcement; Tenant; no coordinates/PII/provider raw facts | Tracking owns detection/evidence; published Fleet lookup is conservative UNKNOWN until source-time classification authority exists; V1/V2 remain ineligible | US-49, US-50, US-52 | CS06 frontend next; production activation separately needs verified powertrain and engine-running mappings | V104 migration/API/security and retained evaluator evidence pass | Technical fixtures may close software only; physical evidence/operator sign-off required for story acceptance |
...
US-51 D1-D11 and CS01–CS05 are complete. Canonical V3 has a verified durable V101 history path,
...
The queue is the approved `CS06 frontend` boundary.
