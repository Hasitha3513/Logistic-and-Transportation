...
The highest-leverage current story is **US-73 — Manage External Integrations**. Its frozen boundary is now implemented with one governed outbound JSON-file adapter, controlled-sandbox evidence, and provider-neutral reliability without duplicating the P1-01 outbox. It remains acceptance-pending while technical closure and independent final acceptance run; later stories may separately approve payroll exchange, billing/accounting handoff, fuel-card imports, telematics, DMS/OCR, push/webhooks, and third-party failure handling.
...
| US-73 | Manage External Integrations | justified `integration` bounded context | IMPLEMENTATION_COMPLETE / ACCEPTANCE_PENDING | P0 | A | H/H/H/H/H | Identity, Tenancy, Audit, US-75, P1-01; domain-owned mappings | Controlled filesystem sandbox; no vendor | IMPLEMENTED: governed outbound JSON file; environment-backed credential-reference port; versioned declarative mapping; five-attempt retry; read-only reconciliation | YES | new admin API; internal durable fact contract; no public inbound API | OPERATOR UI | RBAC/SoD; Tenant; secret/path/payload redaction; future sensitive types rejected | Connectivity/reliable exchange only; no business meaning; reuses P1-01, no cloned outbox | US-78 decisions | V61 and 6/6 real Chromium controlled-sandbox evidence; technical closure pending | COMPLETE — decision and implementation evidence cover owner, adapter, security, persistence, API/UI and controlled sandbox | Configure/test/enable/exchange/status/retry pass; failures never appear synced; real sandbox/file evidence; DoD-C pending independent acceptance |
...
1. `US-73-EXTERNAL-INTEGRATIONS-TECHNICAL-CLOSURE-001` → independent final acceptance (implementation complete; count unchanged).
...
`US-73-EXTERNAL-INTEGRATIONS-TECHNICAL-CLOSURE-001`

It must implement only the frozen governed outbound JSON-file capability; it must not promise every ecosystem named in the mind map.
