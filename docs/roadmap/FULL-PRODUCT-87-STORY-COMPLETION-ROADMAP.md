...
**Wave A is 2 / 2 COMPLETE / CLOSED.** US-73 provides one accepted governed outbound JSON-file adapter with controlled-sandbox evidence. US-78 is independently accepted with Routing and Delivery flowing through one Operations lifecycle while preserving source ownership and P1-01 durability. US-37 product decisions are frozen; the next task is `US-37-FUEL-PERFORMANCE-IMPLEMENTATION-001`.
...
| US-37 | Analyze Fuel Performance | `fuel` (publishes `FuelPerformanceQuery`) | PRODUCT_DECISIONS_FROZEN / IMPLEMENTATION_NOT_STARTED | P1 | B | H/M/M/M/M | US-31/32/33/34/36, Driver, Trip, Fleet, Reporting, Tenant | NONE | Frozen: distance/engine-hour metrics; 7/30/90/custom≤365-day windows; explicit quality; same-vehicle prior-period baseline; compatible peers; deterministic 20% deviation and repeated 30% possible-leakage indicators; non-punitive language; no ML | LIKELY — permission seed only; no analytics table/version reservation | six read-only `/fuel/performance` query routes | DASHBOARD | new `FUEL_PERFORMANCE_VIEW`; Tenant; privacy-controlled driver performance | On-demand Fuel-owned analytics; published bulk contracts only; no foreign SQL/raw mutation/ranking/event/export/US-38 action | NONE | US-37 implementation | Product decisions frozen in `US-37-FUEL-PERFORMANCE-PRODUCT-DECISIONS-001.md`; implementation must supply source-completeness/data-quality fixtures | Vehicle/driver comparisons and non-accusatory anomaly flags pass; raw data unchanged; DoD-C |
