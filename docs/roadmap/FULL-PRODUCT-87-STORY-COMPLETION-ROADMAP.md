...
**Architecture enabler:** `P1-01` COMPLETE; current Flyway head `V103`
...
| US-51 | Monitor Idle Time | `tracking` | IMPLEMENTATION_IN_PROGRESS / CS04_PREREQUISITE_COMPLETE | P1 | C | M/M/M/M/M | US-48 canonical history/movement, effective device capabilities, Audit; Fuel estimate deferred | Production powertrain authority is absent and production lookup returns UNKNOWN; engine-running activation and final acceptance require verified native device evidence | APPROVED D1–D11: combustion/hybrid; native engine-running; 3 km/h, adjusted 50 m, five minutes/two samples, two-minute gap; no Phase-1 effects/fuel estimate | V103 verified: V102 state/episode/dispatch retained; restart-safe candidate state and immutable 180-day candidate evidence added without provisional episodes; CS05 permissions resequenced to V104 | four approved read-only `/api/v1/tracking/idle-monitoring` routes, not yet implemented | approved OPERATOR state/history/detail UI, not yet implemented | approved `IDLE_MONITOR_VIEW`, `IDLE_EVENT_VIEW`; Tenant; no coordinates/PII/provider raw facts | Tracking owns detection/evidence; published Fleet lookup is conservative UNKNOWN until source-time classification authority exists; V1/V2 remain ineligible | US-49, US-50, US-52 | CS04 evaluator/worker activation next; production activation separately needs verified powertrain and engine-running mappings | CS04 prerequisite migration, populated upgrade, restart/discard/promotion, retention, dedupe and Tenant evidence passes; normal IDLE claims remain disabled | Technical fixtures may close software only; physical evidence/operator sign-off required for story acceptance |
