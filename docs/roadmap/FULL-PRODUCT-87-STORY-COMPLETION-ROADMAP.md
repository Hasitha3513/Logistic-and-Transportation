...
**Current state:** 68 / 87 accepted; 19 / 87 remaining; Wave A is 2 / 2 COMPLETE / CLOSED; Wave B is OPEN
**Architecture enabler:** `P1-01` COMPLETE; Flyway head `V63`
...
`US-35, US-38, US-46, US-47, US-48, US-49, US-50, US-51, US-52, US-53, US-54, US-55, US-72, US-76, US-82, US-84, US-85, US-86, US-87`.

This is 19 stories, so the invariant is `68 + 19 = 87`. `US-88`, `US-89`, and `US-90` are undefined and must not be created. The word *deferred* means scheduled into a governed future wave, not permanently abandoned.
...
**Wave A is 2 / 2 COMPLETE / CLOSED and Wave B is OPEN.** US-73 provides one accepted governed outbound JSON-file adapter with controlled-sandbox evidence. US-78 is independently accepted with Routing and Delivery flowing through one Operations lifecycle while preserving source ownership and P1-01 durability. US-37 final acceptance passes; US-35 acceptance is blocked by Bunker ledger ordering, its forward-migration contract is authorized, and the next task is `US-35-FUEL-CARDS-ACCEPTANCE-REMEDIATION-002`.
...
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| US-35 | Manage Fuel Cards | `fuel` | IMPLEMENTATION_COMPLETE / ACCEPTANCE_BLOCKED | P1 | B | H/H/H/H/H | US-31/32/34/36, Fleet, Driver, Organization, Identity, Tenancy, Audit; US-73 outbound capability inspected but not reused | CONTROLLED_PROVIDER_FIXTURE only; named provider remains future | Implemented: limited card reference; local lifecycle; one Vehicle/Driver binding; limits; canonical JSON import; immutable transactions; deterministic reconciliation/review; no payment/fraud engine | V64; V65 authorized for Bunker ledger order if free | frozen operator card/import/reconciliation API implemented | OPERATOR UI implemented | five permissions; Tenant; masked data; importer/reconciler SoD | Fuel owns local control/evidence only; provider owns financial authority; no foreign persistence | US-46 decisions | NONE | Technical closure passed; final acceptance found nondeterministic Bunker ledger tail; canonical Tenant/Tank `ledgerSequence` authorized | Execute `US-35-FUEL-CARDS-ACCEPTANCE-REMEDIATION-002`, then rerun final acceptance |
...
4. US-35 product decisions COMPLETE → implementation → closure → acceptance.
...
It must implement only the authorized Tenant/Tank Bunker `ledgerSequence` migration, allocation, ordering, and regression proof before US-35 final acceptance is rerun. The frozen Fuel Card contract remains unchanged.
