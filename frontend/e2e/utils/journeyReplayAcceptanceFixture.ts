...
export function seedJourneyReplayEvidence(tenantId:string){
  ...
  const speedStart=new Date(base+120_000).toISOString(),speedConfirmed=new Date(base+121_000).toISOString();
  execute(`INSERT INTO tracking_speed_episode(id,tenant_id,vehicle_id,rule_id,rule_version,threshold_source,effective_threshold_kph,start_source_timestamp,confirmation_source_timestamp,max_observed_speed_kph,eligible_above_threshold_sample_count,severity,repeat_count,first_candidate_position_id,confirming_position_id) VALUES ('${crypto.randomUUID()}','${tenant}','${vehicleId}','${crypto.randomUUID()}',1,'TENANT_CONFIG',80,'${speedStart}','${speedConfirmed}',92,2,'WARNING',0,'${crypto.randomUUID()}','${crypto.randomUUID()}');`);
