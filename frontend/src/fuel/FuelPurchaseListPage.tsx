...
const statuses = ['DRAFT', 'SUBMITTED', 'APPROVED', 'RECEIVED', 'RECONCILED', 'CANCELLED'].map((value) => ({
  value,
...
export default function FuelPurchaseListPage() {
  ...
  const money = (value: number, row: FuelPurchase) =>
  ...
  const columns: TableColumnsType<FuelPurchase> = [
    {
      ...
      render: (v, row) => (
        <Link to={`/fuel/purchases/${row.id}`}>
          <Typography.Text strong>{v}</Typography.Text>
        </Link>
      ),
    },
    { title: 'Date', dataIndex: 'purchaseDate' },
    ...
    },
    {
      title: 'Status',
      dataIndex: 'status',
      render: (v) => <FuelPurchaseStatusTag status={v} />,
    },
    {
    ...
    <Flex vertical gap={18}>
      <Flex justify="space-between" align="center" wrap gap={12}>
        <div>
          <Typography.Text type="secondary">
            Invoice, receipt, variance and reconciliation tracking.
          </Typography.Text>
        </div>
        <Space>
          {hasPermission('FUEL_PURCHASE_CREATE') && (
            <Link to="/fuel/purchases/new">
              <Button type="primary" icon={<PlusOutlined />}>New purchase</Button>
            </Link>
          )}
      ...
      <Card variant="borderless">
        <Flex wrap gap={12} align="center">
          <Input.Search
            aria-label="Purchase or invoice search"
          ...
          <Select
            ...
            allowClear
            options={['DIESEL', 'PETROL', 'ELECTRIC', 'OTHER'].map((value) => ({ value }))}
          ...
          <Select
            ...
            style={{ minWidth: 180 }}
          />
          <DatePicker.RangePicker value={period} onChange={(v) => { setPeriod(v); setPage(1); }} />
        </Flex>
      </Card>
      ...
      <Card>
        <Table
          ...
          locale={{ emptyText: 'No fuel purchases found' }}
        />
      </Card>
    </Flex>
  );
