...
describe('journeyReplayApi',()=>{
  ...
  it('queries selected incident producers without putting evidence selectors in the URL',async()=>{
    vi.mocked(api.post).mockResolvedValue({data:{items:[]}});
    const query={vehicleId:'11111111-1111-4111-8111-111111111111',types:['GEOFENCE'] as const};
    await journeyReplayApi.incidents({...query,types:[...query.types]});
    expect(api.post).toHaveBeenCalledWith('/v1/tracking/journey-replays/incidents/query',{...query,types:['GEOFENCE']},{signal:undefined});
  });
});
