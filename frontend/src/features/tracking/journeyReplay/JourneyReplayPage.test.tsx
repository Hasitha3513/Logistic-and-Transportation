...
vi.mock('./api',()=>({journeyReplayApi:{points:vi.fn(),stops:vi.fn(),incidents:vi.fn()}}));
...
describe('JourneyReplayPage',()=>{
  beforeEach(()=>{permissions.clear();permissions.add('JOURNEY_REPLAY_VIEW');vi.mocked(journeyReplayApi.points).mockResolvedValue(page);vi.mocked(journeyReplayApi.stops).mockResolvedValue({...page,items:[],analyzedPointCount:2,pointCeiling:20000} as ReplayStopsPage);vi.mocked(journeyReplayApi.incidents).mockResolvedValue({...page,items:[],producerStatus:['GEOFENCE']})});
  ...
  it('defaults accepted geofence evidence on and labels opt-in technical producers',async()=>{permissions.add('JOURNEY_REPLAY_INCIDENT_VIEW');renderPage();fireEvent.change(screen.getByLabelText('Vehicle ID'),{target:{value:'11111111-1111-4111-8111-111111111111'}});fireEvent.click(screen.getByRole('button',{name:'Load replay'}));expect(await screen.findByText('Incident overlays')).toBeInTheDocument();await waitFor(()=>expect(journeyReplayApi.incidents).toHaveBeenCalledWith(expect.objectContaining({types:['GEOFENCE']}),expect.anything()));fireEvent.click(screen.getByLabelText('Speed episodes — technical evidence'));expect(await screen.findByText('Technical-only incident evidence')).toBeInTheDocument()});
});
