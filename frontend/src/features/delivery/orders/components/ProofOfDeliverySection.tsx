import { EnvironmentOutlined, FileImageOutlined, ScanOutlined } from '@ant-design/icons';
...
import { useAddDeliveryEvidence, useCreateDeliveryProof, useDeliveryProof, useFinalizeDeliveryProof } from '../hooks/useDeliveryOrders';
...
export function ProofOfDeliverySection({ delivery }: Props) {
  ...
  const proof = useDeliveryProof(delivery.id, canCapture || canView); const create = useCreateDeliveryProof(); const add = useAddDeliveryEvidence(); const finalize = useFinalizeDeliveryProof();
  ...
  return <Card title={<Space>Proof of Delivery <Tag color={current.status === 'FINALIZED' ? 'green' : 'blue'}>{current.status}</Tag></Space>}>
    ...
    <List size="small" bordered dataSource={current.evidence} locale={{ emptyText: 'No signature, photo or barcode evidence yet' }} renderItem={item => <List.Item><Space><Tag>{item.type}</Tag><span>{item.barcodeValue ?? item.originalFilename ?? item.checksum}</span></Space></List.Item>} />
