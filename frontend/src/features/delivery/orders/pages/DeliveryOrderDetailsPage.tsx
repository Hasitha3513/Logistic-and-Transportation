...
export default function DeliveryOrderDetailsPage() {
  ...
  return <Flex vertical gap={18}>
    <Flex justify="space-between" align="start"><div><Typography.Title level={3}>{data.deliveryNumber}</Typography.Title><Tag color={data.status === 'DRAFT' ? 'default' : 'green'}>{data.status.replaceAll('_', ' ')}</Tag></div><Space>{hasPermission('DELIVERY_UPDATE') && <Link to={`/deliveries/${data.id}/edit`}><Button icon={<EditOutlined />}>Edit</Button></Link>}{hasPermission('DELIVERY_ASSIGN') && data.status === 'DRAFT' && <Button type="primary" icon={<CheckCircleOutlined />} loading={validate.isPending} onClick={() => void markReady()}>Validate readiness</Button>}</Space></Flex>
