...
public final class FuelPurchasePolicy {
    ...
    public void validateDraft(FuelPurchase purchase) {
        ...
        if (purchase.fuelType() == null || purchase.fuelType().isBlank())
            validation("FUEL_TYPE_REQUIRED", "Fuel type is required");
        if (purchase.currencyCode() == null || purchase.currencyCode().isBlank())
            validation("FUEL_CURRENCY_REQUIRED", "Currency is required");
    ...
    public void validatePrice(FuelPrice price) {
        if (price.unitPrice() == null || price.unitPrice().signum() <= 0)
            validation("INVALID_FUEL_UNIT_PRICE", "Fuel price must be greater than zero");
        if (price.effectiveFrom() == null)
            validation("FUEL_PRICE_EFFECTIVE_FROM_REQUIRED", "Effective-from date is required");
    ...
    public void requireEditable(FuelPurchase purchase) {
        require(purchase, FuelPurchaseStatus.DRAFT, "FUEL_PURCHASE_NOT_EDITABLE", "Only a DRAFT fuel purchase can be edited");
    }

    public void requireSubmittable(FuelPurchase purchase) {
        require(purchase, FuelPurchaseStatus.DRAFT, "FUEL_PURCHASE_NOT_SUBMITTABLE", "Submit requires a DRAFT fuel purchase");
    }

    public void requireApprovable(FuelPurchase purchase) {
        require(purchase, FuelPurchaseStatus.SUBMITTED, "FUEL_PURCHASE_NOT_APPROVABLE", "Approve requires a SUBMITTED fuel purchase");
    }

    public void requireReceivable(FuelPurchase purchase) {
        require(purchase, FuelPurchaseStatus.APPROVED, "FUEL_PURCHASE_NOT_RECEIVABLE", "Receive requires an APPROVED fuel purchase");
    }

    public void requireReconcilable(FuelPurchase purchase) {
        require(purchase, FuelPurchaseStatus.RECEIVED, "FUEL_PURCHASE_NOT_RECONCILABLE", "Reconcile requires a RECEIVED fuel purchase");
    }
    ...
    public void requireCancellable(FuelPurchase purchase, String reason) {
        ...
        if (reason == null || reason.isBlank())
            validation("FUEL_PURCHASE_CANCELLATION_REASON_REQUIRED", "Cancellation reason is required");
    }
