...
public final class FuelPurchaseService implements FuelPurchaseUseCase {
    ...
    public FuelPurchase create(Command command, String username) {
        return transactions.execute(() -> {
            ...
            validateDuplicateInvoice(purchase, null);
            var saved = purchases.save(purchase);
            append(saved, null, saved.status(), "CREATED", actor, saved.notes(), now);
            return saved;
        });
    }
    ...
    public FuelPurchase update(UUID id, Command command, String username) {
        return transactions.execute(() -> {
            ...
            validateDuplicateInvoice(updated, id);
            var saved = purchases.save(updated);
            append(saved, current.status(), saved.status(), "UPDATED", actor, saved.notes(), saved.updatedAt());
            return saved;
        });
    }
    ...
    public FuelPurchase submit(UUID id, String username) {
        return transition(id, username, "SUBMITTED", null, (current, actor, now) -> {
            policy.requireSubmittable(current);
            validateReferences(current);
            validateInvoice(current);
            validateDuplicateInvoice(current, current.id());
    ...
    public FuelPurchase approve(UUID id, String comment, String username) {
        FuelPurchase approved = transition(id, username, "APPROVED", comment, (current, actor, now) -> {
            policy.requireApprovable(current);
            validateReferences(current);
            validateInvoice(current);
    ...
    public FuelPurchase receive(UUID id, ReceiptCommand command, String username) {
        FuelPurchase received = transition(id, username, "RECEIVED", command.remarks(), (current, actor, now) -> {
            ...
            if (command.receivedQuantity() == null || command.receivedQuantity().signum() <= 0)
                throw new BusinessRuleException("INVALID_FUEL_PURCHASE_RECEIPT", "Received quantity must be greater than zero");
            OffsetDateTime receivedAt = command.receivedAt() == null ? now : command.receivedAt();
            if (receivedAt.isAfter(now.plusMinutes(5)))
                throw new BusinessRuleException("INVALID_FUEL_PURCHASE_RECEIPT", "Received time cannot be in the future");
    ...
    public FuelPurchase reconcile(UUID id, ReconciliationCommand command, String username) {
        FuelPurchase reconciled = transition(id, username, "RECONCILED", command.reconciliationNotes(), (current, actor, now) -> {
            ...
            if (current.receivedQuantity() == null || current.receivedAt() == null)
                throw new BusinessRuleException("FUEL_PURCHASE_NOT_RECONCILABLE", "Receipt information is incomplete");
    ...
    @Override
    public FuelPurchase get(UUID id) {
        return purchases.findById(id).orElseThrow(() -> notFound(id));
    }

    @Override
    public PageResult<FuelPurchase> search(SearchQuery query) {
        return purchases.search(query);
    }

    @Override
    public List<FuelPurchaseHistory> history(UUID id) {
        get(id);
        return history.findByPurchaseId(id);
    }
    ...
    private void validateReferences(FuelPurchase purchase) {
        ...
        if (!vendor.active())
            throw new BusinessRuleException("FUEL_VENDOR_INACTIVE", "Inactive vendor cannot be used for a new fuel purchase");
    ...
    private void validateStation(UUID stationId) {
        ...
        if (!station.active())
            throw new BusinessRuleException("FUEL_STATION_INACTIVE", "Inactive fuel station cannot receive fuel");
    }

    private void validateInvoice(FuelPurchase purchase) {
        if (purchase.invoiceNumber() == null || purchase.invoiceNumber().isBlank())
            throw new BusinessRuleException("FUEL_INVOICE_REQUIRED", "Invoice number is required before submission");
        if (purchase.invoiceDate() == null)
            throw new BusinessRuleException("FUEL_INVOICE_DATE_REQUIRED", "Invoice date is required before submission");
    }
    ...
    private FuelPurchase transition(UUID id, String username, String action, String comment, Transition transition) {
        return transactions.execute(() -> {
            ...
            var current = locked(id);
            var now = OffsetDateTime.now(clock);
            var changed = transition.apply(current, actor, now);
            var saved = purchases.save(changed);
    ...
    private FuelPurchase locked(UUID id) {
        return purchases.findByIdForUpdate(id).orElseThrow(() -> notFound(id));
    }

    private NotFoundException notFound(UUID id) {
        return new NotFoundException("FUEL_PURCHASE_NOT_FOUND", "Fuel purchase not found: " + id);
    }
    ...
    private interface Transition {
        FuelPurchase apply(FuelPurchase current, FuelActorPort.Actor actor, OffsetDateTime now);
    }
}
