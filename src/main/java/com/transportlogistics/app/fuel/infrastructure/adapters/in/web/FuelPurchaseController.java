...
class FuelPurchaseController {
    ...
    FuelPurchaseResponse get(@PathVariable UUID id) {
        return response(purchases.get(id));
    }

    @PutMapping("/fuel-purchases/{id}")
    FuelPurchaseResponse update(@PathVariable UUID id, @Valid @RequestBody FuelPurchaseRequest request, Principal principal) {
        return response(purchases.update(id, request.command(), actor(principal)));
    }

    @PostMapping("/fuel-purchases/{id}/submit")
    FuelPurchaseResponse submit(@PathVariable UUID id, Principal p) {
        return response(purchases.submit(id, actor(p)));
    }

    @PostMapping("/fuel-purchases/{id}/approve")
    FuelPurchaseResponse approve(@PathVariable UUID id, @RequestBody(required = false) ApprovalRequest r, Principal p) {
        return response(purchases.approve(id, r == null ? null : r.comment(), actor(p)));
    }

    @PostMapping("/fuel-purchases/{id}/receive")
    FuelPurchaseResponse receive(@PathVariable UUID id, @Valid @RequestBody ReceiptRequest r, Principal p) {
        return response(purchases.receive(id, r.command(), actor(p)));
    }

    @PostMapping("/fuel-purchases/{id}/reconcile")
    FuelPurchaseResponse reconcile(@PathVariable UUID id, @Valid @RequestBody ReconciliationRequest r, Principal p) {
        return response(purchases.reconcile(id, r.command(), actor(p)));
    }

    @PostMapping("/fuel-purchases/{id}/cancel")
    FuelPurchaseResponse cancel(@PathVariable UUID id, @Valid @RequestBody CancellationRequest r, Principal p) {
        return response(purchases.cancel(id, r.reason(), actor(p)));
    }

    @GetMapping("/fuel-purchases/{id}/history")
    List<FuelPurchaseHistory> history(@PathVariable UUID id) {
        return purchases.history(id);
    }
    ...
    @ResponseStatus(HttpStatus.CREATED)
    FuelPrice createPrice(@Valid @RequestBody FuelPriceRequest r) {
        return prices.create(r.command());
    }

    @PutMapping("/fuel-prices/{id}")
    FuelPrice updatePrice(@PathVariable UUID id, @Valid @RequestBody FuelPriceRequest r) {
        return prices.update(id, r.command());
    }
    ...
    record FuelPurchaseRequest(@NotNull UUID vendorId, UUID fuelStationId, @NotBlank @Size(max = 40) String fuelType,
                               @NotNull LocalDate purchaseDate, @Size(max = 100) String invoiceNumber,
        ...
        FuelPurchaseUseCase.Command command() {
            return new FuelPurchaseUseCase.Command(vendorId, fuelStationId, fuelType, purchaseDate, invoiceNumber, invoiceDate, quantity, unitPrice, taxRate, otherCharges, currencyCode, notes);
        }
    }
    ...
    record ReceiptRequest(@NotNull @DecimalMin("0.0001") BigDecimal receivedQuantity, OffsetDateTime receivedAt,
        ...
        FuelPurchaseUseCase.ReceiptCommand command() {
            return new FuelPurchaseUseCase.ReceiptCommand(receivedQuantity, receivedAt, destinationFuelStationId, deliveryNoteNumber, remarks);
        }
    }

    record ReconciliationRequest(@Size(max = 1000) String reconciliationNotes,
                                 @Size(max = 100) String referenceNumber) {
        FuelPurchaseUseCase.ReconciliationCommand command() {
    ...
    record FuelPriceRequest(@NotNull UUID vendorId, @NotBlank @Size(max = 40) String fuelType,
        ...
        FuelPriceUseCase.Command command() {
            return new FuelPriceUseCase.Command(vendorId, fuelType, effectiveFrom, effectiveTo, unitPrice, currencyCode, active);
        }
    }
    ...
    record FuelPurchaseResponse(UUID id, String purchaseNumber, FuelPurchaseUseCase.VendorReference vendor,
                                ...
                                BigDecimal taxRate, BigDecimal taxAmount, BigDecimal otherCharges,
                                BigDecimal totalAmount,
                                String currencyCode, FuelPurchaseStatus status,
                                ...
                                UUID reconciledBy,
                                OffsetDateTime reconciledAt, String reconciliationNotes, String reconciliationReference,
                                String notes,
