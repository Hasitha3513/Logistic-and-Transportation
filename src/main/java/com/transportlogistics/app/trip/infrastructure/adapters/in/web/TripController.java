...
class TripController {
    ...
    Trip start(@PathVariable UUID id, @RequestBody(required = false) StartRequest r, Principal principal) {
        return trips.transition(id, new TripCommand.Start(r == null ? null : r.startOdometerKm(),
                r == null ? null : r.startEngineHours()), actor(principal));
    }
    ...
    Trip complete(@PathVariable UUID id, @RequestBody(required = false) CompleteRequest r, Principal principal) {
        return trips.transition(id, new TripCommand.Complete(r == null ? null : r.endOdometerKm(),
                r == null ? null : r.completionRemarks(), r == null ? null : r.endEngineHours()), actor(principal));
    }
    ...
    @GetMapping("/trips/{id}/distance")
    TripDistanceResponse distance(@PathVariable UUID id) {
        var d = trips.getDistance(id);
        return new TripDistanceResponse(d.tripId(), d.vehicleId(), d.startOdometer(), d.endOdometer(),
                d.distanceKm(), d.status(), d.meterResetEncountered(), d.notes());
    }
    ...
    record TripDistanceResponse(
            UUID tripId,
            UUID vehicleId,
            java.math.BigDecimal startOdometer,
            java.math.BigDecimal endOdometer,
    ...
    record StartRequest(Double startOdometerKm, Double startEngineHours) {
    }

    record CompleteRequest(Double endOdometerKm, String completionRemarks, Double endEngineHours) {
    }
