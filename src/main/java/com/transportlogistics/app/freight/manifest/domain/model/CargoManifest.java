...
public record CargoManifest(UUID id, String manifestNumber, UUID freightOrderId, String freightOrderNumber,
    ...
    public CargoManifest finalizeManifest(List<FreightOrderLookup.ExpectedLine> expected, String actor, OffsetDateTime now) { requireMutable(); var failures=validate(expected); if(!failures.isEmpty()) throw new BusinessRuleException("CARGO_MANIFEST_INCOMPLETE", failures.stream().map(ManifestValidationFailure::message).collect(Collectors.joining("; "))); return new CargoManifest(id,manifestNumber,freightOrderId,freightOrderNumber,items,version,createdAt,now,createdBy,actor,now,actor); }
