...
@Component
final class JdbcRouteDeviationManagementSupportAdapter
        implements RouteDeviationManagementSupportPort {
    private final JdbcTemplate jdbc;

    JdbcRouteDeviationManagementSupportAdapter(JdbcTemplate jdbc) {
        this.jdbc = jdbc;
    }
    ...
    public Claim claim(UUID tenantId, String scope, String key, String requestHash,
        ...
        UUID claimId = UUID.nameUUIDFromBytes((tenantId + "|ROUTE_DEVIATION|" + scope + "|" + key)
        ...
        int inserted = jdbc.update("""
                ...
                VALUES(?,?,?,'ROUTE_DEVIATION_COMMAND_CLAIMED',?,?,?,?) ON CONFLICT(id) DO NOTHING
                """, claimId, tenantId, actorId, "ROUTE_DEVIATION_" + scope, targetId,
                detail, Timestamp.from(now));
        ...
        return jdbc.query("""
                ...
                WHERE id=? AND tenant_id=? AND action='ROUTE_DEVIATION_COMMAND_CLAIMED'
                """, row -> {
    ...
    public void complete(UUID claimId, long resultVersion) {
        if (jdbc.update("""
                ...
                WHERE id=? AND action='ROUTE_DEVIATION_COMMAND_CLAIMED'
                """, Long.toString(resultVersion), claimId) != 1) {
            throw new IllegalStateException("Route-deviation command claim is unavailable");
        }
    }

    @Override
    public void audit(UUID tenantId, UUID actorId, String action, String targetType,
            UUID targetId, String safeDetail, Instant now) {
        jdbc.update("""
                INSERT INTO tracking_audit_event(
                  id,tenant_id,actor_id,action,target_type,target_id,safe_detail,occurred_at)
                VALUES(?,?,?,?,?,?,?,?)
                """, UUID.randomUUID(), tenantId, actorId, action, targetType, targetId,
                safeDetail, Timestamp.from(now));
    }
