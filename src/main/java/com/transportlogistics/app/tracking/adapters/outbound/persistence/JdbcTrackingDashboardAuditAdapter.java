...
final class JdbcTrackingDashboardAuditAdapter implements TrackingDashboardAuditPort {
    ...
    public void record(UUID tenantId, UUID actorId, String correlationId, String action,
        ...
        String detail = "correlationId=" + safe(correlationId)
                + ";filterCategories=" + filterCategories.stream().sorted().toList()
                + ";requestedPageSize=" + requestedPageSize + ";resultCount=" + resultCount
                + ";includedSections=" + includedSections.stream().sorted().toList()
                + ";sourceStatuses=" + sourceStatuses.stream().sorted().toList();
