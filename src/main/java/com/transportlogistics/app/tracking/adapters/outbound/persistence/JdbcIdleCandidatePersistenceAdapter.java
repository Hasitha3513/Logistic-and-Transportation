...
final class JdbcIdleCandidatePersistenceAdapter implements IdleCandidatePersistencePort {
    ...
    private boolean exists(Evidence e){return jdbc.queryForObject("SELECT count(*) FROM tracking_idle_candidate_evidence WHERE tenant_id=? AND candidate_id=? AND source_timestamp=? AND dedupe_identity=?",Integer.class,e.tenantId(),e.candidateId(),ts(e.sourceTimestamp()),e.dedupeIdentity())>0;}
