...
public final class TrackingService implements TrackingUseCase {
 ...
 @Override public State latest(UUID t,UUID v,Instant now){var stored=store.state(t,v,now);if(live==null)return stored.orElseThrow(()->notFound("TRACKING_VEHICLE_NOT_FOUND"));var projected=live.find(t,v);if(projected.isEmpty())return stored.orElseThrow(()->notFound("TRACKING_VEHICLE_NOT_FOUND"));var hybrid=state(projected.get(),now);if(stored.isEmpty()||stored.get().latestTrusted()==null||hybrid.latestTrusted().sourceTimestamp().isAfter(stored.get().latestTrusted().sourceTimestamp()))return hybrid;return stored.get();}
