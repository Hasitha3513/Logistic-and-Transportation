...
public interface EmailNotificationSenderPort {
    SendResult send(SendRequest request) throws InterruptedException;

    record SendRequest(UUID notificationId, String idempotencyKey, String from, String to,
                       String subject, String plainTextBody, Duration timeout) {
        public SendRequest {
            ...
            Objects.requireNonNull(plainTextBody);
    ...
    record SendResult(boolean accepted, String providerMessageId, EmailDeliveryErrorCategory errorCategory,
        ...
        public SendResult {
            if (accepted && errorCategory != null) throw new IllegalArgumentException("Accepted result cannot have an error");
            if (!accepted && errorCategory == null) throw new IllegalArgumentException("Rejected result requires an error category");
        }
