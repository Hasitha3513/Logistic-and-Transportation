...
![Database](https://img.shields.io/badge/TimescaleDB%20%2F%20Flyway-V105-8a63d2.svg?style=for-the-badge&logo=postgresql)
...
> - **US-51 State:** `TECHNICALLY_COMPLETE / PRODUCTION_SOURCE_ACTIVATION_PENDING / ACCEPTANCE_PENDING`; CS01–CS07 and independent technical closure pass at V105, while production Fleet eligibility remains `UNKNOWN` and production engine-running mappings remain disabled.
> - **Immediate Next Action:** No executable US-51 task identifier is authorized. Retain its activation/physical-acceptance hold until authoritative Fleet classification and verified native engine-running evidence support a governed activation decision.
...
> [!NOTE]
> Stories `US-88`, `US-89`, and `US-90` are strictly undefined. The story accounting register is bounded exactly from `US-01` through `US-87` (`73 COMPLETE + 14 REMAINING = 87`).
...
| **Full Product** | **Waves A–E** | Integration/Exceptions; Fuel/Finance; GPS; Compliance/Mobile; Analytics/Integrity/Resilience/Risk | 22 | 7 / 22 | 🟡 `IN PROGRESS — WAVE C ACTIVE` |
...
| `US-51` | Monitor Idle Time | Authoritative engine-running versus movement duration; fuel estimate deferred | 🟡 `TECHNICALLY_COMPLETE / PRODUCTION_SOURCE_ACTIVATION_PENDING / ACCEPTANCE_PENDING` | Consolidated closure PASS at V105; production Fleet classification, native engine-running mapping and physical/operator acceptance remain separate gates |
...
| `US-55` | Handle GPS Edge Cases | Provider-independent signal quality, tamper/battery/power evidence, invalid/poor coordinates, duplicate/late/out-of-order/clock drift, outages, disconnection, stale state, recovery and cross-provider/version dedupe | 🔴 `TECHNICALLY_COMPLETE / IMPLEMENTATION_COMPLETE_ACCEPTANCE_BLOCKED_EXTERNAL_SYSTEM` | V100 retained; field matrix is 0 PASS / 0 FAIL / 9 blocked; final acceptance remains open while independent `US-55-TRACCAR-ADAPTER` work proceeds |
...
| `US-87` | Detect User Risk | Explainable override, missing-field, fraud, shared-login and delayed-reporting controls | 🟡 `IMPLEMENTATION_IN_PROGRESS / CS01_COMPLETE / CS02_AUTHORIZATION_PENDING` | First wave approved by the user for all five authority functions; framework-neutral exact signal/domain contracts pass 10 focused tests; no persistence, producer, runtime evaluation, API, permission or UI yet |
...
Queue Head:     NONE AUTHORIZED — separately authorize US-87-DETECT-USER-RISK-CS02-PERSISTENCE-001 exact DDL and migration boundary
...
5. **Wave E — Analytics, integrity, resilience, disruption and user risk:** US-87 first-wave approval is recorded and CS01 framework-neutral Identity contracts are complete. CS02 persistence is not yet authorized, and the feature remains non-usable and advisory-only with no producer, runtime evaluation, API, permission, UI or enforcement. Current runtime has no generic audit/workflow feed, MFA, device-bound session evidence or automatic enforcement capability. US-85, US-84, US-82 and US-86 retain their documented dependencies.
