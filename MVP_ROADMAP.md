...
> - **US-51 Selection:** D1–D11 and CS01–CS04 are complete. V103 provides restart-safe candidate persistence and the approved evaluator now owns normal durable `IDLE` claims; the Fleet query still returns `UNKNOWN` in production.
> - **Immediate Next Action:** Execute the approved `CS05 V104 APIs/RBAC/audit` boundary; preserve all source-activation and physical-acceptance holds.
...
| `US-51` | Monitor Idle Time | Authoritative engine-running versus movement duration; fuel estimate deferred | 🟡 `IMPLEMENTATION_IN_PROGRESS / CS04_COMPLETE` | V103 candidate persistence and evaluator/worker activation verified; production mappings and physical acceptance remain separately gated |
...
Queue Head:     CS05 V104 APIs/RBAC/audit
...
3. **Wave C — GPS and telematics:** US-48, US-50, US-52, US-53, US-54 and US-55 physical acceptance are on independent external-prerequisite holds; US-49 is accepted. US-51 D1-D11 and CS01–CS04 are complete; CS05 V104 APIs/RBAC/audit is next while production engine-running activation and physical acceptance remain separate gates.
