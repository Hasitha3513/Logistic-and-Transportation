...
![Database](https://img.shields.io/badge/TimescaleDB%20%2F%20Flyway-V103-8a63d2.svg?style=for-the-badge&logo=postgresql)
...
> - **US-51 Selection:** D1–D11 and CS01–CS03 are complete. The verified CS04 prerequisite slice adds V103 restart-safe candidate persistence and a conservative Fleet powertrain query that returns `UNKNOWN` in production; normal `IDLE` claims remain disabled.
> - **Immediate Next Action:** Resume `US-51-MONITOR-IDLE-TIME-CS04-EVALUATOR-001` to implement the approved D1–D5 evaluator and activate its worker only after it functions; preserve all source-activation and physical-acceptance holds.
...
| `US-51` | Monitor Idle Time | Authoritative engine-running versus movement duration; fuel estimate deferred | 🟡 `IMPLEMENTATION_IN_PROGRESS / CS04_PREREQUISITE_COMPLETE` | V103 candidate persistence verified; evaluator/worker activation, production mappings and physical acceptance remain separately gated |
...
Queue Head:     US-51-MONITOR-IDLE-TIME-CS04-EVALUATOR-001
...
3. **Wave C — GPS and telematics:** US-48, US-50, US-52, US-53, US-54 and US-55 physical acceptance are on independent external-prerequisite holds; US-49 is accepted. US-51 D1-D11 and CS01–CS03 plus the CS04 V103 prerequisite persistence slice are complete; the CS04 evaluator is next while production engine-running activation and physical acceptance remain separate gates.
